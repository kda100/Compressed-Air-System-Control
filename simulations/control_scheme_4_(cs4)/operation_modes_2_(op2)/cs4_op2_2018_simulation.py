...
the sheet 'air_volume_stacked_2018' and iterates over the column that contains the 12 bar compressed air data for 2018. For each
...
based on a compressor control sequencing logic defined by 'setpoints' and 'active compressors' in the program. The program then writes the 
...
excel workbook called 'cs4_op2_2018_results.xlsx' located in the same directory as the script.

***************************************************************************************************************************************
2018
Maximum Flow of System - FAD (m3/hr) = 8672
Maximum Air Demand of Plant - FAD (m3/hr) = 7690
...
s9-s10                       [C6, C4, C5]
s11-s12                      [C1, C4, C6, C5]
s12-s13                      [C1, C2, C4, C6, C5]
s13-s14                      [C1, C2, C3, C4, C6, C5]

C1 = OnOff: max_power = 125, max_flow = 947
C2 = OnOff: max_power = 125, max_flow = 947
C3 = OnOff: max_power = 125, max_flow = 947
C4 = OnOff: max_power = 177, max_flow = 1326
C5 = VariableSpeed: max_power = 384, max_flow = 2604
C6 = OnOff: max_power = 274, max_flow = 1902
...
C1 = cm1.OnOff(125, 947) #create OnOff compressor object for C1
C2 = cm1.OnOff(125, 947) #create OnOff compressor object for C2
C3 = cm1.OnOff(125, 947) #create OnOff compressor object for C3
C4 = cm1.OnOff(177, 1326) #create OnOff compressor object for C4
C5 = cm1.VariableSpeed(384, 2604) #create VariableSpeed compressor object for C5
C6 = cm1.OnOff(274, 1902) #create OnOff compressor object for C6
...
Air_Volume_Stacked_2018 = Compressed_Air_Data_12_Bar['air_volume_stacked_2018'] #calls sheet 'air_volume_stacked_2018' in workbook
for row in range(1,8257):
    air_demand = Air_Volume_Stacked_2018.cell(row = row, column = 7).value #iterates over all rows in column
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s1 = compressors[0].maximum_flow #setpoint
    s2 = compressors[3].maximum_flow #setpoint
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s2 = compressors[3].maximum_flow #setpoint
    s3 = compressors[0].maximum_flow + compressors[1].maximum_flow #setpoint
    
    if s2<air_demand<=s3: #boundary
        active_compressors = [compressors[3], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C4_power_output.append(C4.power_output(control_input, air_demand, active_compressors)) #appends C4 power output reading to C4_power_output list
        C4_air_output.append(C4.air_output(control_input, air_demand, active_compressors)) #appends C4 air output reading to C4_air_output list
        C5_power_output.append(C5.power_output(control_input, air_demand, active_compressors)) #appends C5 power output reading to C5_power_output list
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s3 = compressors[0].maximum_flow + compressors[1].maximum_flow #setpoint
    s4 = compressors[5].maximum_flow #setpoint
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s4 = compressors[5].maximum_flow #setpoint
    s5 = compressors[0].maximum_flow + compressors[3].maximum_flow #setpoint
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s5 = compressors[0].maximum_flow + compressors[3].maximum_flow #setpoint
    s6 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[2].maximum_flow #setpoint
        
    if s5<air_demand<=s6: #boundary
        active_compressors = [compressors[0], compressors[3], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C1_power_output.append(C1.power_output(control_input, air_demand, active_compressors)) #appends C1 power output reading to C1_power_output list
        C1_air_output.append(C1.air_output(control_input, air_demand, active_compressors)) #appends C1 air output reading to C1_air_output list
        C4_power_output.append(C4.power_output(control_input, air_demand, active_compressors)) #appends C4 power output reading to C4_power_output list
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s6 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[2].maximum_flow #setpoint
    s7 = compressors[0].maximum_flow + compressors[5].maximum_flow #setpoint
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s7 = compressors[0].maximum_flow + compressors[5].maximum_flow #setpoint
    s8 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[3].maximum_flow #setpoint
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s8 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[3].maximum_flow #setpoint
    s9 = compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    
    if s8<air_demand<=s9: #boundary
        active_compressors = [compressors[0], compressors[1], compressors[3], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C1_power_output.append(C1.power_output(control_input, air_demand, active_compressors)) #appends C1 power output reading to C1_power_output list
        C1_air_output.append(C1.air_output(control_input, air_demand, active_compressors)) #appends C1 air output reading to C1_air_output list
        C2_power_output.append(C2.power_output(control_input, air_demand, active_compressors)) #appends C2 power output reading to C2_power_output list
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s9 = compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    s10 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[5].maximum_flow #setpoint
    
    if s9<air_demand<=s10: #boundary
        active_compressors = [compressors[3], compressors[5], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C4_power_output.append(C4.power_output(control_input, air_demand, active_compressors)) #appends C4 power output reading to C4_power_output list
        C4_air_output.append(C4.air_output(control_input, air_demand, active_compressors)) #appends C4 air output reading to C4_air_output list
        C5_power_output.append(C5.power_output(control_input, air_demand, active_compressors)) #appends C5 power output reading to C5_power_output list
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s10 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[5].maximum_flow #setpoint
    s11 = compressors[0].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    
    if s10<air_demand<=s11: #boundary
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s11 = compressors[0].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    s12 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    
    if s11<air_demand<=s12: #boundary
        active_compressors = [compressors[0], compressors[3], compressors[5], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C1_power_output.append(C1.power_output(control_input, air_demand, active_compressors)) #appends C1 power output reading to C1_power_output list
        C1_air_output.append(C1.air_output(control_input, air_demand, active_compressors)) #appends C1 air output reading to C1_air_output list
        C4_power_output.append(C4.power_output(control_input, air_demand, active_compressors)) #appends C4 power output reading to C4_power_output list
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s12 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    s13 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[2].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
        
    if s12<air_demand<=s13: #boundary
        active_compressors = [compressors[0], compressors[1], compressors[3], compressors[5], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C1_power_output.append(C1.power_output(control_input, air_demand, active_compressors)) #appends C1 power output reading to C1_power_output list
        C1_air_output.append(C1.air_output(control_input, air_demand, active_compressors)) #appends C1 air output reading to C1_air_output list
        C2_power_output.append(C2.power_output(control_input, air_demand, active_compressors)) #appends C2 power output reading to C2_power_output list
    ...
    compressors = [C1, C2, C3, C4, C5, C6] #compressor objects list
    s13 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[2].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow #setpoint
    s14 = compressors[0].maximum_flow + compressors[1].maximum_flow + compressors[2].maximum_flow + compressors[3].maximum_flow + compressors[5].maximum_flow + compressors[4].maximum_flow v
        
    if s13<air_demand<=s14: #boundary
        active_compressors = [compressors[0], compressors[1], compressors[2], compressors[3], compressors[5], compressors[4]] #active_compressors for compressor.power_output and compressor.air_output
        control_input = True #control input for compressor.power_output and compressor.air_output
        C1_power_output.append(C1.power_output(control_input, air_demand, active_compressors)) #appends C1 power output reading to C1_power_output list
        C1_air_output.append(C1.air_output(control_input, air_demand, active_compressors)) #appends C1 air output reading to C1_air_output list
        C2_power_output.append(C2.power_output(control_input, air_demand, active_compressors)) #appends C2 power output reading to C2_power_output list
...
Control_Scheme_Results = openpyxl.load_workbook('cs4_op2_2018_results.xlsx') #opens results workbook in current directory
...
Control_Scheme_Results.save('cs4_op2_2018_results.xlsx') #saves results workbook
