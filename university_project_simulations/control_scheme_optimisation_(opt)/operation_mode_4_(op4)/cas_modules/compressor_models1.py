...
class FixedControl(Compressor): #two-states of control
    ...
    def power_output(self, control_input, air_demand, active_compressors):
        '''
        Takes in three arguments: control_input, air demand and active_compressors and returns the resulting power output for the compressor object
        '''
        if control_input == True: 
    ...
    def air_output(self, control_input air_demand, active_compressors):
        '''
        Takes in three arguments: control_input, air demand and active_compressors and returns the resulting air output for the compressor object
        '''
        if control_input == True: 
...
class VariableControl(Compressor):
    ...
    def discrete_air_output(self):
        ...
        '''
        discrete_air = []
        for air_flow in range(0, self.maximum_flow+1):
    ...
    def discrete_power_output(self):
        ...
        discrete_power = []
        for air_flow in range(0, self.maximum_flow+1):
            discrete_power.append(int(((((self.maximum_power)-(self.minimum_power))/((self.maximum_flow)-(self.minimum_flow)))*air_flow)+self.minimum_flow)) #for each air_flow value the compressor produces the power_output is determined by the linear relationship.
